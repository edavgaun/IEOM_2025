...
def semmantic_drift_plot_plotly(region, year, tf_dfs, tf_idfs,
    ...
    # Quadrant titles
    fig.add_annotation(xref="paper", yref="paper", x=0.2, y=0.975, text="Rare but Telling",
                       font=dict(size=fz, color='#ff7f0e'), showarrow=False)
    fig.add_annotation(xref="paper", yref="paper", x=0.9, y=0.975, text="Frequent + Distinctive",
                       font=dict(size=fz, color='#2ca02c'), showarrow=False)
    fig.add_annotation(xref="paper", yref="paper", x=0.25, y=0.15, text="Noise",
